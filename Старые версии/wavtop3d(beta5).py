...
def create_p3d_from_radp(radp_file, output_p3d, template_p3d=None):
    """
    Создание P3D файла из RADP
    """
    print("=" * 60)
    print("СОЗДАНИЕ P3D ИЗ RADP")
    print("=" * 60)
    
    # Читаем RADP файл
    if not os.path.exists(radp_file):
        print(f"RADP файл не найден: {radp_file}")
        return False
    ...
    # Если есть шаблон P3D, используем его структуру
    if template_p3d and os.path.exists(template_p3d):
        with open(template_p3d, 'rb') as f:
            template_data = f.read()
        
        # Ищем radp в шаблоне
        radp_pos = template_data.find(b'radp')
        if radp_pos != -1:
            # Берем все до radp из шаблона
            if template_data[radp_pos+5:radp_pos+9] == b'RADP':
                p3d_header = template_data[:radp_pos + 9]
            else:
                p3d_header = template_data[:radp_pos + 5]
            
            print("Использован шаблон P3D для структуры")
        else:
            print("Шаблон не содержит radp, создаем базовую структуру")
            p3d_header = create_basic_p3d_header()
    else:
        print("Создана базовая структура P3D")
        p3d_header = create_basic_p3d_header()
    
    # Создаем P3D
    p3d_data = bytearray()
    p3d_data.extend(p3d_header)
    p3d_data.extend(radp_data[9:])  # Пропускаем radp\0RADP
    
    # Обновляем размер файла
    file_size = len(p3d_data)
    p3d_data[4:8] = struct.pack('<I', file_size)
    
    # Сохраняем
    with open(output_p3d, 'wb') as f:
        f.write(p3d_data)
    
    print(f"\n✓ Создан P3D файл: {output_p3d}")
